...
save_dir_path = signalmcmed_dir_path + "/evaluation_test_10min_csfm-base"
...
with open(signalmcmed_dir_path + "/extract-features_csfm-base_10min_ppg.pkl", "rb") as f:
    features_10min_ppg = pickle.load(f)
print(features_10min_ppg.shape)
with open(signalmcmed_dir_path + "/extract-features_csfm-base_10min_ecg_ppg.pkl", "rb") as f:
    features_10min_ecg_ppg = pickle.load(f)
...
for i in range(len(preprocessed_csns)):
    if preprocessed_csns[i] in signalmcmed_csns:
        ...
        features_10min_ecg_ppg_list.append(features_10min_ecg_ppg[i])
...
features_10min_ecg_ppg = np.stack(features_10min_ecg_ppg_list, axis=0)
...
for train_prop_i, train_prop in enumerate(train_props):
    ...
    for run_i in range(number_of_runs):
        ...
        val_features_list_10min_ecg_ppg = []
        for i, csn in enumerate(preprocessed_csns):
            if csn in val_csns:
                age_values = visits_df[visits_df["CSN"] == csn]["Age"].values
                assert len(age_values) == 1
                ...
                val_features_list_10min_ecg_ppg.append(features_10min_ecg_ppg[i])
        ...
        val_features_10min_ecg_ppg = np.stack(val_features_list_10min_ecg_ppg, axis=0)
        print(len(val_ages))
        print(val_features_10min_ecg.shape)
        print(val_features_10min_ppg.shape)
        print(val_features_10min_ecg_ppg.shape)
        ...
        X_val_10min_ecg_ppg = val_features_10min_ecg_ppg
        ...
        for alpha in alpha_values:
            ...
            y_val_pred = pipe.predict(X_val_10min_ppg)
            mae = np.mean(np.abs(y_val - y_val_pred))
            maes_10min_ppg.append(mae)
            pearson_corr, _ = pearsonr(y_val_pred, y_val)
            pearsons_10min_ppg.append(pearson_corr)

            pipe = make_pipeline(StandardScaler(), Ridge(alpha=alpha))
            pipe.fit(X_train_10min_ecg_ppg, y_train)
            y_val_pred = pipe.predict(X_val_10min_ecg_ppg)
            mae = np.mean(np.abs(y_val - y_val_pred))
            maes_10min_ecg_ppg.append(mae)
            pearson_corr, _ = pearsonr(y_val_pred, y_val)
            pearsons_10min_ecg_ppg.append(pearson_corr)
        ...
        #
        best_idx_ecg_ppg = np.argmax(pearsons_10min_ecg_ppg)
        ...
        plt.plot(alpha_values, pearsons_10min_ecg_ppg, "^-", color="C2", label="ECG + PPG -- 10min")
        plt.plot(alpha_values, pearsons_10min_ecg_ppg_mean, "^-", color="C3", label="ECG-only + PPG-only (mean of features) -- 10min")
        plt.ylabel("Pearson corr")
        plt.xlabel("alpha")
        plt.xscale("log")
        plt.legend()
        plt.savefig("%s/Ridge_val_pearsons_combined_10min_age_alpha_values_train_prop%.2f_run_i%d.png" % (save_dir_path, train_prop, run_i), dpi=300)
        plt.close(1)
        ...
        auprcs_10min_ppg = []
        rocaucs_10min_ppg = []
        auprcs_10min_ecg_ppg = []
        rocaucs_10min_ecg_ppg = []
        ...
        for c_value in c_values:
            ...
            pipe.fit(X_train_10min_ppg, y_train)
            y_val_pred = pipe.predict(X_val_10min_ppg)
            y_val_proba = pipe.predict_proba(X_val_10min_ppg)[:, 1]
            rocauc = roc_auc_score(y_val, y_val_proba)
            rocaucs_10min_ppg.append(rocauc)
            auprc = average_precision_score(y_val, y_val_proba)
            ...
            pipe.fit(X_train_10min_ecg_ppg, y_train)
            y_val_pred = pipe.predict(X_val_10min_ecg_ppg)
            y_val_proba = pipe.predict_proba(X_val_10min_ecg_ppg)[:, 1]
            rocauc = roc_auc_score(y_val, y_val_proba)
            rocaucs_10min_ecg_ppg.append(rocauc)
            auprc = average_precision_score(y_val, y_val_proba)
            auprcs_10min_ecg_ppg.append(auprc)
        ...
        #
        best_idx_ecg_ppg = np.argmax(rocaucs_10min_ecg_ppg)
        ...
        plt.plot(c_values, rocaucs_10min_ecg_ppg, "^-", color="C2", label="ECG + PPG -- 10min")
        plt.plot(c_values, rocaucs_10min_ecg_ppg_mean, "^-", color="C3", label="ECG-only + PPG-only (mean of features) -- 10min")
        plt.ylabel("AUROC")
        plt.xlabel("C")
        plt.xscale("log")
        plt.legend()
        plt.savefig("%s/LogisticRegression_val_rocaucs_combined_10min_sex_c_values_train_prop%.2f_run_i%d.png" % (save_dir_path, train_prop, run_i), dpi=300)
        plt.close(1)
        ...
        print("min: %.2f" % np.min(test_potassiums))
        ...
        print(test_features_10min_ecg_ppg.shape)
        print("test label stats:")
        print("min: %.2f" % np.min(test_calciums))
        ...
        print(test_features_10min_ecg_ppg.shape)
        print("test label stats:")
        print("min: %.2f" % np.min(test_egfrs))
        ...
        print(test_features_10min_ecg_ppg.shape)
        print("test label stats:")
        print("min: %.2f" % np.min(test_glucoses))
        ...
        print(test_features_10min_ecg_ppg.shape)
        print("test label stats:")
        print("min: %.2f" % np.min(test_hemoglobins))
        ...
        print(test_features_10min_ecg_ppg.shape)
        print("test label stats:")
        print("min: %.2f" % np.min(test_albumins))
        ...
        print(test_features_10min_ecg_ppg.shape)
        print("test label stats:")
        print("min: %.2f" % np.min(test_sodiums))
